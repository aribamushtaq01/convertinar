...
const useCases = [
  {
    ...
    accentFrom: "#81bb26", accentTo: "#81bb26",
    subtitleColor: "#81bb26", btnFrom: "#81bb26", btnTo: "#81bb26",
  ...
  return (
    <>
      <div className="flex flex-col">
        ...
        <a
          href={uc.href}
          className="inline-flex flex-row-reverse items-center gap-2.5 px-6 md:px-[30px] py-2.5 md:py-3 rounded-full text-[0.9rem] md:text-[1rem] font-bold text-black w-fit no-underline transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(129,187,38,0.3)]"
  ...
  return (
    <section className="w-full bg-[#0a0a0a] py-12">
      <div className="max-w-[1400px] mx-auto px-4 md:px-10">
        ...
        >
          Who Uses <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#81bb26] to-[#81bb26]">ConvertInAr</span>
        </motion.h2>

        {/* MOBILE Tabs */}
        <div className="md:hidden mb-8 -mx-4">
          <div className="flex gap-4 overflow-x-auto px-6 pb-4 [scroll-snap-type:x_mandatory] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {useCases.map((uc, i) => {
              ...
              return (
                <button
                  ...
                  className="relative flex-shrink-0 w-[48vw] h-[160px] rounded-[20px] bg-transparent border-none p-0 cursor-pointer transition-all duration-400 [scroll-snap-align:center]"
                  style={{
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      filter: isActive ? `drop-shadow(0 12px 24px #81bb2666)` : 'none'
                    }}
                  />
                  <div className="absolute bottom-0 inset-x-0 flex items-center justify-center">
                    <span className={`text-[0.75rem] font-bold px-4 py-2 rounded-full backdrop-blur-md transition-all duration-300 ${isActive ? 'bg-white/10 text-white border border-white/20' : 'bg-black/40 text-white/70 border border-transparent'}`}>
                      {uc.tabLabel}
                    </span>
                  </div>
                </button>
        ...
        <div className="hidden md:flex justify-center mb-12 w-full">
          <div ref={tabsRef} className="flex md:gap-8 lg:gap-12">
            {useCases.map((uc, i) => {
              const isActive = i === activeDisplayIndex;
              return (
                <button
                  ...
                  className="relative flex flex-col-reverse items-center justify-start text-center cursor-pointer flex-shrink-0 w-[160px] lg:w-[180px] h-[200px] border-none bg-transparent p-0 transition-all duration-300"
                  style={{ opacity: isActive ? 1 : 0.5 }}
                >
                  <img
                    src={uc.thumbnail}
                    alt={uc.label}
                    className="absolute inset-x-0 top-0 w-full pointer-events-none transition-all duration-400 object-contain"
                    style={{
                      height: 'calc(100% - 2.5rem)',
                      transform: isActive ? `scale(${uc.thumbScale * 1.05}) translateY(-8px)` : `scale(${uc.thumbScale * 0.85})`,
                      filter: isActive ? `drop-shadow(0 20px 30px #81bb2655)` : 'none'
                    }}
                  />
                  <div className="relative z-10 w-full flex flex-col items-center justify-end h-10 mb-2">
                    <span className={`text-[0.9rem] lg:text-[1rem] font-bold transition-colors duration-300 ${isActive ? 'text-white' : 'text-white/70'}`}>{uc.tabLabel}</span>
                    <div className={`h-1 rounded-full mt-2 transition-all duration-300 ${isActive ? 'w-8' : 'w-0'}`} style={{ backgroundColor: '#81bb26' }} />
                  </div>
                </button>
              );
            })}
        ...
        <div className="relative min-h-[420px] md:min-h-[380px]">
          {exitingCase && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center absolute inset-0 opacity-0 pointer-events-none -translate-y-4 transition-all duration-450 ease-[cubic-bezier(0.4,0,0.2,1)]">
              <PanelContent uc={exitingCase} videoRef={(el) => { videoRefs.current[visibleIndex] = el; }} />
            </div>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center relative opacity-100 translate-y-0 transition-all duration-450 ease-[cubic-bezier(0.4,0,0.2,1)]">
